...
def disable_telemetry():
    for device in devices:
        control_file_name="/sys/devices/pci0000:" + device[1] + "/0000:" + device[1] + ":00.0/telemetry/control"
        command = "echo 0 > " + control_file_name
        try:
            str(subprocess.check_output(command, shell=True))
        except:
            break
...
def helper():
    print("This is script for qat telemetry use cases\n")
    print("Usage: telemetry_monitor.py -s <control value> -m\n")
    print("    -s, --set-val=<control value>    0 or 1, 0 is telemetry disabled and 1 is to enable")
    print("    -m, --monitor    start qat monitoring work like top")
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hs:m", ["help", "set-val=", "monitor"])
    except getopt.GetoptError as err:
        print("command error, err details: %s" % err)
        helper()
        sys.exit(2)
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            helper()
            sys.exit()
        elif opt in ("-s", "set-val"):
            set_val = arg
        elif opt in ("-m", "--monitor"):
            is_monitor = True
    ctrl_val = set_val.strip()

    if not ctrl_val and (is_monitor is True):
        print("no contorl value is set while asking for monitor is not allowed, use -h for help.")
        sys.exit(2)

    if ctrl_val is not None and ctrl_val.isdigit():
        enable_val = int(ctrl_val)
        if enable_val not in (0, 1):
            print("not valid control value, should be 0 or 1")
            sys.exit(2)
        
        if  enable_val == 0 and (is_monitor is True):
            print("QAT telemetry control not set 1 while asking for monitor. exit!")
            sys.exit(2)

        if enable_val == 0:
            get_devices()
            disable_telemetry()
            print("QAT telemetry disabled")
    
        if enable_val == 1:
            get_devices()
            enable_telemetry()
            print("QAT telemetry enabled")

        if enable_val == 1 and (is_monitor is True):
            curses.wrapper(pbar)
    else:
        print("command not correct, please check helper info below.")
        helper()
        sys.exit(2)